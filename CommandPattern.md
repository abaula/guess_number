...
Паттерн Command — мощный инструмент построения гибких систем, позволяющий целиком вынести логику каждого метода в отдельный класс.

Совмещение Command с Dependency Injection (DI) даёт дополнительные преимущества в архитектуре приложений.
...
В адаптированной реализации команды (Command) принимают все зависимости через DI-контейнер, что устраняет жёсткое связывание между командой и внешними сервисами. Такой подход особенно часто встречается в .NET, Java и других современных экосистемах имеющие развитые средства внедрения инверсии управления.

Однако сами параметры передаются непосредственно в метод исполнения команды. Такой подход позволяет с наименьшими трудозатратами реализовывать передачу обрабатываемых данных между командами - конвеер, поток данных, data workflow.
...
Адаптация паттерна Command с применением Dependency Injection (DI) усиливает разделение ответственностей и облегчает масштабирование и тестирование команд, но имеет ряд преимуществ и недостатков.
...
- **Тестируемость**: все внешние сервисы легко замокать или подменить при написании Unit и Integration тестов.
...
*Хотя здесь важно отметить, что это целиком зависит от того насколько понятно весь функционал приложения разбит на отдельные функции.*
...
Грамотное проектирование команд и сервисов требует всегда опираться на SOLID-принципы — особенно на "dependency inversion" и "single responsibility":
   ...
   - Задача должна описыватся в логике максимально понятной человеку.

#### Пересоленный SOLID

Важно отметить вредность излишнего увлечения абстракциями как структурными, так и алгоритмическими.

Чрезмерное увлечение абстракциями — как структурными (классы, интерфейсы, уровни архитектуры), так и алгоритмическими — негативно сказывается на качестве кода и производительности команды. Это проявляется в усложнении понимания, сопровождения и внедрения изменений в проект.
...
- Понижается читаемость: новые члены команды испытывают трудности при погружении в систему из-за абстракций ради самих абстракций.
...
- Использование универсальных алгоритмических абстракций там, где достаточно было бы конкретных реализаций, мешает оптимизации и делает код менее прозрачным для анализа и дебага.
...
- Для решения каждой мелкой задачи создаётся самостоятельная сущность, лишь увеличивая количество вспомогательного кода.

**Практические рекомендации**

- Не выводить абстракции заранее без повторяющихся кейсов, а рефакторить по мере появления дублирования поведения.
...
DI от Microsoft доступен начиная с .NET Core.

Поддерживает внедрение Lazy<T> "из коробки".

Можно просто указать зависимость в конструкторе как Lazy<T>, и DI-контейнер создаст объект только при первом обращении через Lazy.Value.

Для более продвинутого поведения (например, ленивое создание прокси) существуют дополнительные библиотеки (LazyProxy).

**Autofac**

Встроенная поддержка Lazy<T>.

Позволяет регистрировать зависимости как ленивые и внедрять их как Lazy<T>.

Существует библиотека LazyProxy.Autofac для более гибкого ленивого внедрения.

**Unity Container**

Поддерживает Lazy<T> либо через встроенный функционал, либо с помощью LazyProxy.Unity.

LazyProxy.Unity позволяет регистрировать сервисы как ленивые с проксированием вызовов.

**Другие библиотеки и решения**

Существует библиотека LazyProxy.ServiceProvider, которая добавляет ленивую инициализацию для стандартного Microsoft DI контейнера с проксированием.

Аналоги для Autofac и Unity также есть, позволяющие избежать "грязного" кода с Lazy<T>, сохраняя чистоту и прозрачность архитектуры.

### Как избегать циклических зависимостей

Циклические зависимости — одна из самых частых ловушек в системах с DI и паттерном Command, особенно кода команды зачастую расширяют свою функциональность и начинают ссылаться друг на друга или на сервисы, которые зависят от них же.
